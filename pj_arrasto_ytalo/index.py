...
m1 = get_input("Digite o valor da massa da Partícula 1 (padrão: 1.0): ", 1.0)  # Massa da partícula 1
m2 = get_input("Digite o valor da massa da Partícula 2 (padrão: 1.0): ", 1.0)  # Massa da partícula 2
m3 = get_input("Digite o valor da massa da Partícula 3 (padrão: 1.0): ", 1.0)  # Massa da partícula 3
...
while True: # Programa fecha e o codigo para
    tela.fill((175,175,175))
    for event in pygame.event.get():
        if event.type == QUIT:
            pygame.quit()
            exit()
    dt = clock.tick(60) / 1000.0  # Delta de tempo no SI
    a1 = g - c1 * v1**2 / m1  # Aceleração do círculo 1
    v1 += a1 * dt  # Atualiza a velocidade
    y1 += v1 * dt  # Atualiza a posição

    a2 = g - c2 * v2**2 / m2  # Aceleração do círculo 2
    v2 += a2 * dt
    y2 += v2 * dt

    a3 = g - c3 * v3**2 / m3  # Aceleração do círculo 3
    v3 += a3 * dt
    y3 += v3 * dt
